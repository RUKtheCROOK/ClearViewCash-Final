...
export default function Budgets() {
  ...
  const load = useCallback(async () => {
    ...
    const monthStart = new Date(Date.UTC(year, monthIdx, 1)).toISOString().slice(0, 10);
    ...
    setTxns60d(txns);
    const thisMonth = txns.filter((t) => t.posted_at >= monthStart);
    setSpent(computeSpentByCategory(thisMonth));
  }, [activeSpaceId, sharedView, restrictToOwnerId, year, monthIdx]);
  ...
  // Suggest one un-budgeted category from this month's spend
